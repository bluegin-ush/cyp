...
create table Archivo (entidadCrediticia_id bigint, fechaGeneracion timestamp(6), id bigint not null, estado varchar(255) check (estado in ('GENERADO','ENVIADO','PROCESADO','ERROR')), archivo oid, detalleErrores oid, primary key (id));
...
create table Factura (total numeric(38,2), vtoCae date, archivo_id bigint, fecha timestamp(6), id bigint not null, loteFactura_id bigint, nroComprobante bigint, socio_id bigint, cae varchar(255), estado varchar(255) check (estado in ('EMITIDA','PAGADA','CANCELADA','PARCIALMENTE_CANCELADA')), qr TEXT, tipo varchar(255) check (tipo in ('C')), primary key (id));
...
create table LoteFactura (anio integer not null, mes integer not null, fechaGeneracion timestamp(6), id bigint not null, primary key (id));
...
INSERT INTO entidadCrediticia(id,tipo, nombre, archivo, cuit, contacto) VALUES     (1,'tarjeta-credito','tarjeta de credito visa - débito automático',true,0,'sin datos de contacto'), (2,'tarjeta-credito','tarjeta de credito master card - débito automático',true,0,'sin datos de contacto'), (3,'tarjeta-credito','tarjeta de credito tdf - débito automático',true,0,'sin datos de contacto'), (4,'tarjeta-credito','tarjeta de credito visa',false,0,'sin datos de contacto'), (5,'tarjeta-credito','tarjeta de credito master card',false,0,'sin datos de contacto'), (6,'tarjeta-credito','tarjeta de credito american express',false,0,'sin datos de contacto'), (7,'tarjeta-credito','tarjeta de credito tdf',false,0,'sin datos de contacto'), (8,'tarjeta-debito','tarjeta de débito maestro',false,0,'sin datos de contacto'), (9,'tarjeta-debito','tarjeta de débito visa electron',false,0,'sin datos de contacto');
