...
create table Factura (total numeric(38,2), vtoCae date, archivo_id bigint, fecha timestamp(6), id bigint not null, loteFactura_id bigint, nroComprobante bigint, socio_id bigint, cae varchar(255), estado varchar(255) check (estado in ('PROCESO','EMITIDA','PAGADA','CANCELADA','PARCIALMENTE_CANCELADA')), qr TEXT, tipo varchar(255) check (tipo in ('C')), primary key (id));
...
create table LoteFactura (anio integer not null, estado smallint check (estado between 0 and 3), mes integer not null, progreso integer not null, fechaGeneracion timestamp(6), id bigint not null, idFacturasEmitidas bigint array, idFacturasErroneas bigint array, primary key (id));
